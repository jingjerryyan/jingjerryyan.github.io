---
permalink: /
title: "Home"
author_profile: false
redirect_from: 
  - /about/
  - /about.html
---

<div class="hero hero--full hero--profile">
  <canvas class="hero__canvas"></canvas>
  <div class="hero__grid"></div>
  <div class="hero__aurora" aria-hidden="true"></div>

  <div class="hero__frame">
    <nav class="hero__topnav" aria-label="Primary">
      <a href="/">Home</a>
      <a href="/research/">Research</a>
      <a href="/cv-json/">C.V.</a>
      <a href="/discussions/">Discussions</a>
      <a href="/teaching/">Teaching</a>
      <a href="/decoding/">Decoding</a>
    </nav>

    <div class="hero__body">
      <aside class="hero__card">
        <img class="hero__avatar" src="/images/profile.png" alt="Jing Yan" />
        <h2 class="hero__card-name">Jing Yan</h2>
        <p class="hero__card-cjk">闫璟 &middot; 閆璟</p>
        <ul class="hero__card-meta">
          <li><i class="fas fa-fw fa-location-dot" aria-hidden="true"></i><a href="https://maps.app.goo.gl/YotxriCCWCpzb8xa8" target="_blank" rel="noopener">Philadelphia, PA</a></li>
          <li><i class="fas fa-fw fa-envelope" aria-hidden="true"></i><a href="mailto:jing.yan0001@temple.edu">Email</a></li>
          <li><i class="fas fa-fw fa-building-columns" aria-hidden="true"></i><a href="https://www.fox.temple.edu/directory/jing-yan" target="_blank" rel="noopener">School Directory</a></li>
          <li><i class="fas fa-fw fa-file-lines" aria-hidden="true"></i><a href="https://papers.ssrn.com/sol3/cf_dev/AbsByAuth.cfm?per_id=7959104" target="_blank" rel="noopener">SSRN Author</a></li>
          <li><i class="fas fa-fw fa-graduation-cap" aria-hidden="true"></i><a href="#" target="_blank" rel="noopener">Google Scholar</a></li>
        </ul>
      </aside>

      <div class="hero__main">
        <h1 class="hero__name">Jing Yan</h1>
        <p class="hero__role">Ph.D. Student in Finance</p>
        <p class="hero__bio">I am a Second Year Ph.D. student in Finance at Temple University's Fox School of Business, supervised by <a href="https://sites.google.com/view/samuel-rosen/">Prof. Samuel Rosen</a>, <a href="https://sites.google.com/view/gurdipbakshi1/Home">Prof. Gurdip Bakshi</a> and <a href="https://sites.google.com/site/xiaohuigaobakshi/">Prof. Xiaohui Gao Bakshi</a>. I hold an M.Sc. in Finance from The Hong Kong University of Science and Technology (2023&ndash;2025) and a B.Sc. in Mathematics and Statistics from Hong Kong Baptist University (2019&ndash;2023). My current research studies how equity markets priced U.S. stablecoin legislation.</p>
        <p class="hero__interests"><span>Research Interests</span>Empirical Banking &nbsp;&middot;&nbsp; Financial Regulation &nbsp;&middot;&nbsp; FinTech and Digital Assets</p>
      </div>
    </div>
  </div>
</div>

<script src="/assets/js/hero.js"></script>
